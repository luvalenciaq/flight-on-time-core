...
public class FeatureEngineeringServiceImpl implements FeatureEngineeringService {
    ...
    public Map<String, OnnxTensor> transformar(Flight flight) {
        ...
        try {
            ...
            WeatherDataDTO weather = weatherService.getWeatherData(flight.getOrigen(), fecha);

            features.put("temp_range_f",
                    OnnxTensor.createTensor(env, new float[][]{{weather.tempRangeF()}}));
            features.put("dewpoint_range_f",
                    OnnxTensor.createTensor(env, new float[][]{{weather.dewpointRangeF()}}));
